...
fn generate_wesl(
    ...
    if ir
        .values
        .iter()
        .any(|value| matches!(value.instruction, MaterialIrInstruction::NormalMap { .. }))
    {
        source.push_str(include_str!("shaders/aestra_normal_map.wesl"));
    }
...
fn instruction_expression(
    ...
    let expression = match &value.instruction {
        ...
        MaterialIrInstruction::NormalMap {
            ...
            "aestra_normal_map({}.xyz, {}, {}, {}, {}, {})",
            value_name(*sample),
            value_name(*strength),
            value_name(*flip_y),
            value_name(*normal),
            value_name(*tangent),
            value_name(*bitangent)
        ),
...
fn hash_instruction(fingerprint: &mut FingerprintBuilder, instruction: &MaterialIrInstruction) {
    match instruction {
        MaterialIrInstruction::NormalMap {
            ...
            fingerprint.byte(28);
            for value in [sample, strength, flip_y, normal, tangent, bitangent] {
