...
fn aestra_trail_vertex(vertex_index: u32, draw_index: u32) -> SpriteVertexData {
    ...
    let count = aux[base * 3u + 1u];
    if particle_alive(particles[base]) == 0u || count == 0u {
        return output;
    }
    ...
    if cap {
        segment = select(0u, count - 1u, head_cap);
    }
    if segment >= count {
        return output;
    }
