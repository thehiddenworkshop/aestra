...
### AAA-scale dense sweep — measured (RTX 4070 SUPER / Vulkan)

The B00x suite topped out at 100k **alive** (the big-capacity scenarios are sparse),
which is 1–2 orders of magnitude below AAA hero-effect / scene-total particle counts.
A dedicated dense sweep (`benchmarks/sweep-scenarios/scale_*.ron`, generated by
`gen_scale.ps1` — burst = capacity, lifetime = duration = 120s so **100% occupancy**
holds across the whole capture; archived under
`benchmarks/gpu-baselines/scale-sweep-b24ce16/`) pushes real alive counts to the
~4.19M single-dispatch ceiling. Same-session `--gpu-bench` p50 (ms):

| Alive | `simulate` GPU | `transparent_pass` GPU | render ÷ sim |
| ---: | ---: | ---: | ---: |
| 50k | 0.40 | 0.97 | 2.4× |
| 100k | 0.73 | 1.88 | 2.6× |
| 250k | 1.71 | 4.61 | 2.7× |
...
The 4M sim distribution is tight (min 2.208 / p50 2.218 / max 2.337 / **stddev 0.006**),
so these are precise, not noise. Findings — several of them corrections:

- **The analytical sim does NOT hit a throughput wall at scale.** It scales ~linearly
  to ~250k, then **plateaus** — 4M dense simulates in **2.2 ms**. The marginal cost
  1M→4M is ~0.05 ns/particle: the 4070's parallelism absorbs the reconstruction once
  the kernel saturates. **This refutes the earlier linear extrapolation** (which
...
- **This weakens M7's throughput motivation.** The incremental backend was framed as
  ...
  persistent-state semantics, but **not** as the answer to dense-throughput on
  capable hardware — that answer is render-side.
- **Hard ceiling: ~4.19M particles/effect** in the current 1-D dispatch
  (`max_compute_workgroups_per_dimension 65535 × 64`, `gpu.rs:740`). Beyond it the
  engine clamps `max_particles`. Targeting >4M/effect needs multi-dispatch or a 2-D
  dispatch — a concrete AAA-readiness item.
- **Non-monotonic point (2M = 1.47 ms < 1M = 2.07 ms), reproducibly.** A
  workgroup-tiling/occupancy alignment effect on the SM grid, not measurement error
  (distributions are tight). Single points at scale still carry real GPU-clock
