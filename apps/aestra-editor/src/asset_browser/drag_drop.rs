...
fn eligible(
    ...
    if catalog.content().asset_operation_suffix(source).is_none() {
        return Err("Only effects, materials and material functions can be moved or copied. Folder moves are not supported yet.".into());
    }
    let entry = catalog
        .content()
        .source(source)
...
fn enter(
    ...
    session.status = valid.as_ref().map_or_else(
        ...
        |_| "Release to choose Move Here or Copy Here".into(),
    );
...
fn choose(
    ...
    let suffix = catalog
        .content()
        .asset_operation_suffix(choice.source)
        .unwrap();
    let name = entry
        .name
        .to_string_lossy()
        ...
        .unwrap()
        .to_owned();
    let retarget = !choice.copy
        && catalog.content().asset_for_source(choice.source)
    ...
    io::enqueue(&mut commands, guard.clone(), move || {
        enum Plan {
            Move(aestra_project::content::operations::RenamePlan),
        ...
        io::completion(move |world| {
            ...
            let result = plan.and_then(|plan| match plan {
                Plan::Move(plan) => plan
                    .apply()
                    .map(|result| (result.asset, result.destination)),
                Plan::Copy(plan) => plan
                    .apply()
                    .map(|result| (result.asset, result.created_source)),
            });
            match result {
                Ok((asset, path)) => {
                    prepared.refresh();
...
mod tests {
    ...
    #[test]
    fn drop_preflight_rejects_stale_colliding_same_folder_and_folder_sources() {
        let root = tempfile::tempdir().unwrap();
    ...
    fn folder_tree_and_content_drops_offer_move_copy_and_escape_cancels() {
        for tree in [false, true] {
            for view in [
                ...
                assert!(choices.contains(&true) && choices.contains(&false));
